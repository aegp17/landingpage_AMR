...
- `sections/` — page sections rendered in `App.vue` order: HeroSection, ServicesSection, ProcessSection, TeamSection, SkillsSection, ResearchSection, CtaBanner, ContactSection. `ProcessSection` is a self-contained interactive showcase of the Client Value Delivery Framework (3 phase pills + 7-stage stepper with active-stage detail panel + governance band + 3 service-modality cards); all copy lives under `t.process.*` in the i18n dictionaries — the source-of-truth for the framework content is `docs/Client_Value_Delivery_Framework_Agentic_AMR.pdf`.
