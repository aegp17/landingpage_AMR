...
scripts/       stamp-build.mjs (run by the deploy workflow), build-icons.mjs, build-foods.mjs
```
...
**The food table.** `foods.json` holds 85 common foods with their calories per 100 g and a few household portions. It is generated by `scripts/build-foods.mjs` from **USDA FoodData Central, SR Legacy** (public domain): each food is pinned by its FDC id and checked against the description it should have, so a dataset update cannot quietly change what "rice" means. Every entry keeps its `fdcId` and the original USDA description. The Spanish aliases (`arroz`, `pollo`, `frejol`) are ours, for searching only — the app's own text stays English. To regenerate:
